...
Each topic follows: Concept → Why it matters → HCL/CLI syntax → Exam traps
...
When you need to manage resources across multiple instances of the same provider — for example, two AWS regions — you use provider **aliases**. You can declare the same provider multiple times, each with a different `alias`, and then reference the correct one per resource.
...
- A provider block without an `alias` is the **default** for that provider. Resources use it automatically unless you specify `provider = <name>.<alias>`.
- The `provider` meta-argument on a resource uses dot notation: `provider = aws.west` — not a string.
- After adding a new provider block, always re-run `terraform init`.
...
> State is created only when `terraform apply` runs successfully for the first time. Running `terraform init` and then `terraform apply` with no existing state file: Terraform applies normally, creates all resources fresh, writes a new `terraform.tfstate`. No error — this is normal first-run behaviour.
...
The core Terraform workflow is a repeatable pipeline: **Write → Init → Plan → Apply → Destroy**. Every Terraform operation follows this sequence.
...
terraform validate  — check syntax and config validity
    ↓
terraform fmt       — apply canonical formatting
    ↓
terraform plan      — preview what will change (dry run)
    ↓
terraform apply     — execute changes against real infrastructure
    ↓
...
- `plan` always runs before `apply` — either explicitly or implicitly as part of `apply`
- `apply` shows the plan and asks for confirmation before making any changes
- The workflow is the same regardless of which provider or cloud you use
- Each command is idempotent: running apply on an unchanged config makes zero changes

### Exam Traps

- `terraform init` does NOT create state — state is created on first successful `apply`
- `terraform validate` does NOT check if cloud resources exist — only syntax and references
- `terraform plan` makes zero changes to real infrastructure — it is always a dry run
...
> It upgrades already-installed provider plugins to the **latest version that still satisfies your version constraints**. It does NOT ignore constraints — it works within them and updates `.terraform.lock.hcl` with the new version and checksums. Use when a newer patch or minor version is available and you want to pull it in.
...
**Rule:** Only use depends_on when a dependency cannot be expressed through a direct attribute reference.
...
| 2 | **Terraform Cloud workspace variables** | Variables defined in the workspace. |
| 3 | **`terraform.tfvars` and `*.auto.tfvars`** | Automatically loaded by Terraform. |
| 4 | **`-var-file=custom.tfvars`** | Explicitly loaded with the CLI flag. |
| 5 | **`-var="key=value"`** | Explicit variable assignment from the CLI. |
| 6 — Highest | **`TF_VAR_<NAME>` environment variables** | Overrides lower-precedence sources. |

Auto-loaded files include `terraform.tfvars`, `terraform.tfvars.json`, `*.auto.tfvars`, and `*.auto.tfvars.json`. Any other `.tfvars` file must be explicitly specified with `-var-file`.
...
- `list` supports index access (`[0]`); `set` does not — sets are unordered and unique.
- `map` enforces one type for all values; `object` lets each attribute have its own type.
- `for_each` requires a **map or set**, NOT a list — use `toset()` to convert.
...
> }
> ```
> After `toset()`, `each.key` is the string value itself. Removing one entry only destroys that one resource — unlike `count` which cascades.
...
>   tags = { ship = "millennium-falcon", pilot = "han" }
>   lifecycle {
>     ignore_changes = [tags]
>   }
> }
> ```
> Tags are applied at create time. After that, any external changes (by the bot) are ignored by Terraform.
> Do NOT use `ignore_changes = all` — that ignores every attribute change on the resource, which is too broad. Overusing it reduces parallelism.
...
Pinning module versions prevents unexpected breaking changes when module maintainers release updates. Without a version constraint, `terraform init` always downloads the latest version — risky in production.
...
- `version` is only valid for **registry** module sources — not local paths or Git URLs.
- For Git sources, use `?ref=` to pin a tag: `git::https://github.com/org/repo.git?ref=v1.2.0`
- After changing a module version, run `terraform init` to download the new version.

> **💡 Exam Q30 — Version constraint `~> 3.2.0`:**
> Available: 3.2.0, 3.2.5, 3.3.0, 4.0.0 → Terraform downloads **3.2.5**
> `~> 3.2.0` = `>= 3.2.0, < 3.3.0`. Only the patch digit can increment. 3.3.0 and 4.0.0 are blocked.
...
The **local backend** is Terraform's default. State is stored in `terraform.tfstate` in the working directory. A backup of the previous state is automatically saved as `terraform.tfstate.backup`.

### Characteristics

- State stored locally on disk as JSON
- No sharing — only one person can safely use it at a time
- No built-in locking mechanism (OS-level file locking only)
- Not suitable for team use — anyone running apply could overwrite another's state

### When Local Backend is Acceptable

- Solo projects or learning environments
- Configurations that don't need collaboration
- When state doesn't contain sensitive data

### Exam Traps

- Never commit `terraform.tfstate` to version control — it often contains plaintext secrets
- `terraform.tfstate.backup` holds the prior state — useful for manual recovery
- Deleting the state file does NOT delete your infrastructure, but Terraform will think nothing exists

> **💡 Exam Q7 — Local backend, tfstate deleted, run `terraform plan`:**
> Terraform sees no state → plans to **CREATE all resources** as if starting fresh. No error is thrown. It simply has no memory of the existing infrastructure. Fix with `terraform import`.
...
**State locking** prevents multiple users or CI/CD runs from applying changes simultaneously, which would corrupt the state file. When a `plan` or `apply` starts, Terraform acquires a lock. If another operation tries to start while the lock is held, it fails with a lock error.
...
|---|---|
| local | OS-level file locking |
| s3 | DynamoDB table (must configure separately) |
| azurerm | Azure Blob lease |
| gcs | GCS object locking |
| HCP Terraform | Built-in (no extra config needed) |

### Exam Traps

- The **S3 backend does NOT natively lock state** — you must pair it with a DynamoDB table
- `terraform force-unlock LOCK_ID` — releases a stuck lock; use with caution
...
A **remote backend** stores Terraform state in a shared, persistent location — required for teams. Remote backends also enable state locking and encryption.
...
- `-reconfigure` reinitialises without copying state; `-migrate-state` copies existing state to the new backend

> **💡 Exam Q10 — Remote backend, tfstate deleted, run `terraform plan`:**
> Same as local backend — Terraform plans to **CREATE all resources**. The remote backend just stores state. If the file is gone, Terraform has no memory of existing infrastructure.
...
`terraform import` brings existing real-world resources under Terraform management by writing them into state — without destroying and recreating them.
...
## 7b. Use the CLI to inspect and manipulate state

### Concept

Terraform provides the `terraform state` subcommand family for inspecting, moving, and removing resources from state without destroying real infrastructure.
...
### Also: Terraform Workspaces

A **workspace** is a named, isolated state instance. The `default` workspace always exists.
...
Reference in config: `terraform.workspace == "prod" ? "t3.large" : "t3.micro"`

### Exam Traps

- `terraform state rm` removes from Terraform management — the real cloud resource still exists
- `terraform state mv` is for renaming/refactoring — use it instead of destroy+recreate
- CLI workspaces are different from HCP Terraform workspaces — CLI workspaces share config, HCP workspaces are fully isolated
...
### Sentinel Policies and Policy as Code
...
### Run Triggers and Cross-Workspace Data Sharing
...
HCP Terraform integrates with the Terraform CLI via the `cloud` block. This enables remote state storage, remote plan/apply execution, and team collaboration from your local workflow.
...
| Mode | Where plan/apply runs | State location |
|---|---|---|
| **Remote** | HCP Terraform infrastructure | HCP Terraform |
...
```bash
terraform init -migrate-state   # copies existing local/remote state to HCP Terraform
```

### Exam Traps

- HCP Terraform is the new name for Terraform Cloud — exam may use either
- `terraform login` stores the token locally — does not embed it in config files
- In Remote execution mode, cloud credentials must be set as Environment Variables in the workspace — not on your local machine
- The free tier includes state storage, VCS integration, and private module registry
