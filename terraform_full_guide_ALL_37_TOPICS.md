...
### Three Ways to Establish a Provider Dependency

The exam asks this as a "select three" question. A provider dependency can be established in three ways:

**1. `required_providers` block** — Explicitly declares the provider, its source, and version constraint. This is the recommended approach and the only way to pin a version.
...
**2. `provider` configuration block** — Configuring a provider with a `provider "aws" { ... }` block implicitly establishes a dependency on that provider, even without a `required_providers` entry. Terraform infers the provider is needed.
...
**3. Resource or data source block** — Simply using a resource type like `resource "aws_instance"` implicitly tells Terraform the `aws` provider is required. Terraform infers the provider from the resource type prefix. No explicit `provider` block or `required_providers` entry is strictly needed for Terraform to know which provider to download.

```hcl
resource "aws_instance" "web" {   # "aws_" prefix → aws provider is required
  ami           = "ami-12345678"
  instance_type = "t3.micro"
}
```

In practice you should always use `required_providers` for explicit version control, but the exam tests that you know all three mechanisms count as establishing a dependency.
...
The `.terraform/` directory is created in your working directory when you run `terraform init`. It is Terraform's local cache — it holds everything Terraform needs to operate that is too large or environment-specific to commit to version control.

It contains:
- `.terraform/providers/` — downloaded provider plugin binaries
- `.terraform/modules/` — downloaded module source code
- `.terraform/terraform.tfstate` — backend configuration state (not your infrastructure state)

**Provider plugin location:** Provider plugins are stored at `.terraform/providers/registry.terraform.io/<NAMESPACE>/<TYPE>/<VERSION>/<OS_ARCH>/` within your working directory. For example, the AWS provider on Linux would be at `.terraform/providers/registry.terraform.io/hashicorp/aws/5.31.0/linux_amd64/terraform-provider-aws_v5.31.0`.

The `.terraform/` directory is **not committed to version control** — it is environment-specific and regenerated by `terraform init`. The lock file (`.terraform.lock.hcl`) IS committed because it records which versions were selected.
...
terraform destroy -target=<ADDR>         # destroy one specific resource only
terraform plan -destroy -target=<ADDR>  # preview destroying one specific resource
```

### Targeted Destroy (`-target`)

When you need to decommission a specific resource without touching the rest of your infrastructure, use the `-target` flag. It accepts a resource address and limits the destroy operation to that resource and its dependents.

```bash
# Destroy only a specific Cloud SQL instance
terraform destroy -target=google_sql_database_instance.main

# Destroy a module and everything in it
terraform destroy -target=module.database
```

**Important:** `-target` is a tactical escape hatch — not a routine workflow. Because it only destroys part of the state, it can leave your configuration and state inconsistent if the destroyed resource has dependents that are not also targeted. Always verify with `terraform plan` after a targeted destroy.

The exam scenario for `-target` is: "decommission only X while keeping all other infrastructure running."
...
### Setting Variables via Environment Variables (`TF_VAR_`)

Any shell environment variable prefixed with `TF_VAR_` is automatically read by Terraform as an input variable. The prefix is stripped and the remainder becomes the variable name.

```bash
export TF_VAR_region="us-west-2"      # sets var.region = "us-west-2"
export TF_VAR_db_password="secret"    # sets var.db_password = "secret"

terraform plan   # reads TF_VAR_region and TF_VAR_db_password automatically
```

`TF_VAR_` is the mandatory prefix — without it, Terraform ignores the environment variable. This is commonly used in CI/CD pipelines to inject sensitive values without committing them to `.tfvars` files.
...
**Input validation** adds custom rules to variable blocks. Terraform checks them at the start of the **plan phase** — before any API calls, before resource operations, and before any other processing. If a validation condition fails, Terraform reports the error immediately and stops. The apply phase is never reached.

This timing is exam-tested. The sequence is:

```
User sets variable value (via -var, tfvars, env var, etc.)
        ↓
Terraform evaluates all validation blocks   ← error reported HERE if condition fails
        ↓
terraform plan runs (API calls, diff calculation)
        ↓
terraform apply runs
```

So if a user sets `instance_count = 15` and the validation requires `<= 10`, Terraform reports the error during the plan phase before doing anything else.
...
### Passing Output from One Module to Another Module

The root module acts as the wiring layer between child modules. When one child module produces an output that another child module needs as an input, the root module reads the first module's output and passes it as an argument to the second module.

```hcl
# Root module — wires database output into webapp input
...
module "webapp" {
  ...
  connection_string = module.database.connection_string   # output → input
}
```

For this to work, three things must be in place:
1. The `database` module must declare `output "connection_string"` in its `outputs.tf`
2. The `webapp` module must declare `variable "connection_string"` in its `variables.tf`
3. The root module passes the value using `module.database.connection_string`

Child modules cannot reference each other directly — all cross-module data flows through the root module.

### What Happens When an Output Block is Missing

A common error: you reference `module.network.vpc_id` in your root module, but the `network` module has no `output "vpc_id"` block. Terraform will error during plan:

```
Error: Unsupported attribute
  module.network.vpc_id is not a valid attribute
```

The resource `aws_vpc.main` exists inside the module and has a `.id` attribute — but it is invisible from outside the module. **Resources inside a module are never directly accessible from outside.** The only data that can leave a module is what is explicitly declared in an `output` block. The fix is to add the output to the child module:
...
- **Deleting the state file is the most serious operational mistake in Terraform.** The infrastructure continues running — nothing in the cloud is affected. But Terraform loses its entire mapping between configuration and real resources. On the next `terraform apply`, Terraform sees no state and believes nothing exists, so it will attempt to create every resource from scratch — producing duplicates, conflicts, or failures depending on the resource type. Recovery requires manually re-importing every resource, which is laborious and error-prone. This is why remote backends with versioning (S3 with versioning enabled, HCP Terraform) are critical — they allow state recovery if a file is accidentally deleted.
...
There are two distinct commands for dealing with drift and they do very different things:

**`terraform plan -refresh-only`** — Read-only preview. Queries the real-world state of all resources and shows you how the state file *would* be updated to reflect reality. It makes no changes — not to infrastructure, not to the state file. Use this when you want to see what has drifted without committing to anything yet.

**`terraform apply -refresh-only`** — Actually updates the state file to match the real-world values. Infrastructure is not touched — only the state file is written. Use this when you have reviewed the drift and want to accept the manual changes into state.

```
Manual change made outside Terraform
        |
        ├── terraform plan -refresh-only
        |       Shows: "state would be updated like this"
        |       Changes nothing — safe preview
        |
        └── terraform apply -refresh-only
                Writes the updated values into the state file
                Infrastructure untouched — only state changes
```

This is also different from a regular `terraform plan`, which refreshes state AND compares against your configuration — meaning it would propose reverting the manual change to match config. `plan -refresh-only` only reconciles state; it does not propose reverting anything.
...
### Full Import Workflow — Three Steps

When you need to bring existing manually-created resources under Terraform management without disrupting them, the process has three required steps:

**Step 1 — Write the resource configuration block**
Write a `resource` block in your `.tf` files that describes the existing resource. The resource type and name must match what you will use in the import command. At minimum, include the required arguments.
...
**Step 2 — Run `terraform import` to link state to the real resource**
Run the import command with the resource address (matching your config) and the real resource's ID from the cloud provider.

```bash
terraform import aws_instance.web i-0abc1234def56789
```

This writes an entry into the state file linking `aws_instance.web` to `i-0abc1234def56789`. It does not touch the real resource.

**Step 3 — Run `terraform plan` to verify alignment**
After import, run `terraform plan`. If your configuration accurately describes the real resource, the plan should show no changes. If it shows proposed changes, your config doesn't fully match the real resource's current settings — adjust the config until the plan is clean.
...
This three-step process (write config → import → verify with plan) is the answer to exam questions like "what steps are required to bring existing resources under Terraform management without impacting them."

Terraform 1.5+ added declarative **`import` blocks** as an alternative:
...
Combined with `terraform plan -generate-config-out=generated.tf` to auto-generate the HCL configuration in Step 1.
...
**Q1:** A Sentinel policy with enforcement level "soft-mandatory" fails. What happens?
A) The apply proceeds with a warning in the logs
B) The apply is blocked and cannot proceed under any circumstances
C) The apply is blocked but can be manually overridden by an authorised user
D) The plan is re-run with the policy applied
✅ **Answer: C**

**Q2:** At which stage in the HCP Terraform run pipeline do Sentinel policies execute?
A) Before the plan
B) After the plan but before the apply
C) After the apply
D) Concurrently with the apply
✅ **Answer: B**
...
**Q1:** A developer needs to queue plans and review output for a workspace, but must not be able to apply changes. Which workspace permission level should they receive?
A) Read  B) Plan  C) Write  D) Admin
✅ **Answer: B** — Plan allows queuing plans but not applying.

**Q2:** True or False: In HCP Terraform, permissions should be assigned to individual users rather than teams for maximum security.
✅ **Answer: False** — HCP Terraform's model assigns permissions to teams. Individual user permissions are not the recommended approach.
